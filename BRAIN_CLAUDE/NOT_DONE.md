...
| L2b | **Suites de L2** | 🟡 | Bouton d'interface pour offrir une Graine de Sceau (l'API existe) ; taux D3 dans `BalanceConfig` (asset aujourd'hui lu par personne) ; perception du miroir pour examiner un orifice sans cultivateur confirmé et Qi de talisman *Prolonger la vie* (+40 ans) avec l'axe du miroir (L6) ; autres blessures persistantes (WL-007, seules les blessures du Dao sont enregistrées). ✅ 2026-09-26 : **Qi de talisman** (`TalismanSystem`, `talismans.json` : prières annuelles, rituel de sacrifice, offre de 1 à 3 selon talent et tempérament, bond et trait ; vie, vitesse, Illusions et talent des enfants branchés). Reste : écran du rituel (G6), traits de combat, captifs à sacrifier (L6), Qi de talisman par éclats du miroir. Le rituel sacrifie des **bêtes chassées** (décision 2026-09-26 : jamais un membre ; bête plus puissante = meilleur Qi ; bêtes solitaires ou d'une puissance, dont la mort a un prix ; terrain de chasse sur la carte). Reste : bêtes nommées et combat de chasse, capture risquée (L6). |
...
| L4c | **Suites de L4** | 🟡 | Imagerie et Mandat de Vie ; lien à un trésor, emprunt de lumière ; « Dao mûr » convoité par les plus forts ; corps inhumains ; phénomènes de fondation, à la mort et à l'échec (L6) ; révéler les capacités inconnues (miroir, ruines, échanges) ; mêmes règles pour les cultivateurs des factions (P1) ; ~~constantes d'interprétation de L1-L2 dans `balance.json`~~ (✅ 2026-09-26, section `trials`) ; ~~`KnowledgeBase.Revealed` dans la chronique~~ (✅ 2026-09-26, `KnowledgeView`). ✅ 2026-09-26 aussi : achat de techniques aux puissances et lecture des Partenaires Dao par le miroir (`KnowledgeExchange`), Imagerie (capacité incarnée : moitié de l'XP), corps inhumains (sang doré, gouttelettes électrifiées, hérités), emprunt de lumière (« Miséricordieux »), Dao mûr chassé. Reste : Mandat de Vie, lien à un trésor, phénomènes (L6) ; mêmes règles pour les factions (P1). |
| L4d | ~~Serments du Dao~~ | ✅ | Fait 2026-09-26 : pactes entre deux cultivateurs de toute voie (`oaths.json`), parjure = chemin interrompu ou Démon du Cœur, portes dérobées (tiers, prête-nom, échéance, voile du miroir, purification). Suites : réincarnation/conversion (L7), serments des factions, écran de pacte (G6) ; les clauses « garder le secret » et « ne pas épier » attendent l'espionnage entre personnes et les secrets (aucune action ne les rompt encore). |
| L4b | ~~Routes du Noyau d'Or~~ (§5.9) | ✅ | Fait 2026-09-26 (L4b.1-L4b.3, branche `feature/l4b-golden-core-routes`) : percée en deux étapes, forger l'essence métallique avec la méthode de recherche d'or de la lignée visée (savoir déchiffré par le miroir ; méthode spécialisée pour l'Intercalaire 3+2), puis demander une position : Réalisation d'une lignée libre (le membre en devient détenteur), Surplus (une substitution parmi les cinq), Intercalaire 4+1 ou 3+2 (4e et 5e capacités d'une seule autre lignée). Rester sans position (R7) et retenter plus tard est possible ; une lignée occupée exige la permission du détenteur (tribut, caduque s'il change) ; l'échec de l'une ou l'autre étape fait naître un Démon d'Essence Métallique (mort). Capacités superficielles et greffées pèsent sur la forge, la Vie condensée en dernier aide, l'axiome des positions pénalise la demande. Main Gauche vraie (Yang Suprême, Yin Mineur : `leftHand` dans `fruitions.json`) et fausse (pouvoir prêté par un détenteur, tribut annuel, chute avec le patron). Position affichée au domaine. Valeurs : `balance.json` `goldenCore` (interprétations). Sauvegarde 2.5. |
| L4e | **Suites de L4b** | 🟡 | R6 Intercalaire « pont » 4A+1B=C (table des ponts selon l'état du monde, L6) ; R8 Transfert, Transmutation, Transformation et Reprise de la Fruition ; R9 réincarnation d'un ancêtre Vrai Monarque et Élus du Destin (L6) ; Démon d'Essence Métallique comme menace régionale et sa hiérarchie (L6) ; lignée transformée, empreinte, Désignation de Rang, Trésors de Dharma ; stades 2-4 du Noyau d'Or ; capacités mineures d'anciens Vrais Monarques (à nommer dans les données) ; atmosphère favorable à la percée (L6) ; boutons de l'écran (forger, demander, tribut, déchiffrer : G6) ; mêmes règles pour les Vrais Monarques des factions (P1). ✅ 2026-09-26 : stades 2-4 du Noyau d'Or (affirmation de l'image), Transfert et Transformation, Reprise de la Fruition (Lutte des Cinq Visages), lignée transformée. Reste : Transmutation (source trop vague), R6, R9, démon comme menace, Désignation de Rang et Trésors (L6), capacités mineures (données), boutons (G6), P1. |
| L5 | ~~Carte et factions renommées~~ | ✅ | Fait 2026-09-26 (L5.1-L5.3, branche `feature/l5-map-and-factions`) : `regions.json` (9 États et mers du §7.1, 43 lieux des §7.2-§7.4 avec parent, position relative et voisins ; positions et voisinages interprétés, trois sièges non nommés par le lore aux noms interprétés), maison du clan au Lac Jingshui (`clan.json`) ; `factions.json` remplacé par 35 puissances du lore (3 sectes et 8 portes du Linxi, Portes du Carnage et de la Clarté Révérée, Atoll des Perles Noires, 11 familles du §8, ordres, temples bouddhistes, États) avec type, région, voie, royaume le plus haut et techniques du §2.4 (personnalité, puissance, richesse, relation : interprétations) ; écran « Carte du monde » (encre sur papier, puissances du lieu choisi) ; le smoke finit sur la carte. Les sauvegardes d'avant L5 gardent leurs factions inventées (« sans lieu connu »). 853 tests verts. |
| L5b | **Suites de L5** | 🟡 | Carte peinte Shuimo avec zoom (le centre du Linxi reste dense : quelques étiquettes se chevauchent), positions affinées ; Qi et atmosphère par région (B4, L6) ; territoire et déplacements du clan, frontières mouvantes (§7.4) ; puissances du lore dans les systèmes : l'IA des factions, l'espionnage et les alliances n'utilisent encore que personnalité et puissance, pas la région, la voie ni les techniques (vol de techniques : L3b) ; Monde Souterrain, Grand Song et autres puissances du §12 (L6). ✅ 2026-09-26 (analyse du wiki) : positions de 37 lieux reprises de la carte du Jiangnan, sièges sans nom renommés d'après leur secte ou porte, sixième famille du lac (Kang), 20 figures nommées des puissances (`figures.json`) affichées sur la carte ; noms nouveaux à valider (LORE.md §13.9). ✅ puis : voisins plus intenses dans l'humeur des puissances, vol de manuels par l'espionnage, zoom et déplacement sur la carte. Reste : carte peinte, Qi et atmosphère par région, territoire du clan, IA des factions par voie et techniques (P1). |
| W1 | ~~Validation des noms (analyse du wiki)~~ | ✅ | Validés par l'utilisateur le 2026-09-26 (famille Kang, 16 figures, 4 fondations : LORE.md §13.9). Les 43 techniques citées par les fiches attendent grade et Qi (§6.8b, L3b). |
| L6 | **Chronologie, monde vivant, rejouabilité** | 🟡 | Introduction an 0, faits historiques, statuts initiaux des Fruitions, phénomènes régionaux, graine de monde, conditions de victoire. Démons d'Essence Métallique (LORE.md §6.9, §8) : réclamés par le Monde Souterrain ; choix du joueur à leur naissance (le laisser, sceller ou reprendre l'essence au risque de provoquer le Monde Souterrain, ne rien faire) ; soumission par les forces pseudo-Noyau d'Or (Jade Tressé du clan) ; pot-de-vin d'essence. |
