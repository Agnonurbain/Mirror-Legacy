...
✅ Fait en L3 (2026-09-25) : catalogue `game/data/techniques.json` (les 26 méthodes du §2.4, l'*Intuition du Lotus Blanc*, le *Sutra de l'Aîné qui Frappe à la Cour*, le *Dialogue de Gongye Shu*) et `qi.json` ; `TechniqueRules` (vitesse par grade réglée dans `balance.json`, plafonds du §2.2, secret du Manoir Pourpre, Qi pour entrer en Culture du Qi), `TechniqueLibrary` (savoir du clan, méthode de chaque membre), tâche « Récolte de Qi », déduction du miroir → technique secrète graduée. Le clan Mo commence avec le *Sutra de la Source Claire*, le *Souffle Commun* (acheté avant la partie) et deux portions de *Qi de la Source Claire*.

🔎 Interprétations faites en L3 :
- **Grades en fourchette** : on retient la borne basse compatible avec le royaume suprême (« 4+ » qui mène au Manoir Pourpre → 5 ; « 5+ » → 5 ; « 3+ » → 3 ; *Perle Grise* « 3-4 » → 3 ; *Sutra du Verbe Premier* « 5-6 », Noyau d'Or ? → 6). *Méthode des Six Harmonies* : ni grade ni élément connus → grade 3, famille inconnue. Catégorie non précisée → commune ; « Art secret » → secrète.
- **Qi non nommé par la source** : il porte le nom de sa méthode (*Qi de la Source Claire*…). Nommés par le lore : *Qi Profond du Bélier Variant*, *Qi Coloré de l'Aube*, *Qi de l'Armure de Givre de la Cour*, *Qi de la Poussière Rouge de la Cour* (disparu). La phase L4 rattachera chaque Qi à sa Fruition.
- **Souffle Commun** (« permet à n'importe qui de devenir cultivateur de Qi ») : lu comme *n'importe quel porteur d'orifice*, sinon la Graine de Sceau perdrait son sens. Son Qi mineur est partout : y entrer ne coûte aucune portion. Son prix : grade 2, plafond à la Culture du Qi, et le Qi absorbé lie le cultivateur aux méthodes de ce Qi.
- **Sans manuel** : en Respiration Embryonnaire on respire (« respiration commune », vitesse d'un grade 3) ; au-delà, sans méthode qui couvre son royaume, on ne progresse plus.
- **Entrer en Culture du Qi** coûte une portion du Qi de la méthode. Si le membre n'a pas choisi de méthode, les anciens prennent la meilleure connue dont le Qi est en réserve, **jamais** une méthode de grade 1-2 : seul le patriarche (le joueur) peut l'imposer.
- **Récolte** : dès l'Œil du Sommet (5e chakra, qui perçoit le Qi spirituel) ; une année de travail par portion (vingt pour le *Qi Profond du Bélier Variant* : dix de filaments, dix de raffinage), sur le Qi de la méthode du récolteur, sinon sur le meilleur Qi que le clan sait récolter.
- **Déduction** : grade = qualité moyenne des fragments, +1 à partir de quatre fragments, au plus 6 ; 7+ seulement avec cinq fragments divins (un seul manuel 7+ est connu). La technique est secrète ; une méthode déduite repose sur un Qi récoltable de son élément ; un art s'utilise dès le royaume de son grade (1-2 Respiration, 3-4 Qi, 5-6 Fondation, 7+ Manoir Pourpre) ; un art de déplacement ajoute un pas en combat (deux dès le grade 5).
- **Défauts du *Veilleur du Sentier*** : vitesse ×1,5 en Respiration, ×0,5 en Culture du Qi, durée de vie ×0,95, et aucun dégât contre qui pratique le *Sutra de l'Aîné qui Frappe à la Cour*.

Reste pour plus tard : boutiques des techniques communes (L5-L6), portion de Qi de la fondation immortelle (L4), raffinage du Qi des Sept Terrasses par un démon (L6), *Sutra de la respiration du Yin Suprême* avec le premier éclat (L6), apprentissage des arts et écran de déduction (G6), Qi selon les régions (L5-L6).
...
✅ Fait en L4.2 (2026-09-25) : la percée de la Fondation absorbe **une portion** du Qi de la méthode (§2.5), que l'épreuve réussisse ou non, et forme **la fondation de ce Qi** (`CharacterData.FoundationId`). Les **Partenaires Dao** sont les autres fondations de la lignée. **Consommer** celle d'un membre du clan fait gagner un stade tout de suite, puis scelle toute progression (ni XP, ni épreuve) ; ✅ **décision de l'utilisateur (2026-09-25) : le donneur, sa fondation dévorée, meurt.** On ne dévore qu'un Partenaire **connu** du clan (« comprendre ses Partenaires Dao est vital avant d'entrer dans le Dao »). **Cœur Dao** (💡 validé) : un tempérament héréditaire (dominateur, solitaire, patient, fougueux, rusé, serein ; hérité d'un parent dans 80 % des cas) ; chaque lignée en favorise un (son profil §6.9, sinon son élément ou sa famille) ; aligné ×1,2, mal aligné ×0,9 à partir de la Fondation ; chaque année, 2 % de chances que le cœur s'aligne sur sa fondation.
...
✅ Fait en L4.3-L4.4 (2026-09-25) : la percée s'ouvre au sommet de la Fondation pour une méthode qui porte le secret de montée. Les quatre épreuves se résolvent à chaque phase Percée : **Montée** (échec = mort, `AscentCollapse`) ; **Manifestation**, 6 ans de retraite (✅ décision de l'utilisateur, 2026-09-25 : son échec, « principale cause de chute », **tue**) ; **Grand Vide**, tiré par tranches dans `balance.json` (50 % moins d'un an, 30 % 1-10 ans, 15 % 11-40 ans, 5 % prisonnier à vie) ; **Illusions** (échec = toute la cultivation perdue, durée de vie de mortel). La réussite ouvre le Manoir Pourpre avec la fondation pour première capacité. Un membre en retraite ne prend aucune tâche. **Capacités 2 à 5** : on vise un Partenaire **révélé** de sa lignée (P3), puis on le condense avec l'XP du royaume, une technique connue alignée sur lui et une portion de son Qi ; ou avec des ressources, à moitié de l'XP, mais capacité « superficielle » ; ou par **Greffe du Dao** (la fondation d'un membre du clan, complétée par des minerais spirituels ; ✅ décision de l'utilisateur, 2026-09-25 : le donneur **perd toute sa cultivation et n'a plus que 1 à 5 ans à vivre**). On ne vise qu'une capacité que le lore nomme **et** que le clan connaît. La 4e passe le **Seuil d'Immortalité** (échec = XP perdue). Le stade suit le nombre de capacités.
...
- **Démons d'Essence Métallique** (hiérarchie) : issus d'un **détenteur de Réalisation** → catastrophes rivalisant avec un Vrai Monarque ; issus d'un **échec de montée** → pas forcément faibles (le Ciel avait déjà reconnu l'essence) ; issus de voies anciennes sans position → bien inférieurs.
  - 📚 **Naissance** : à la mort d'un cultivateur, son essence devient **sans maître** ; si personne ne la reprend ni ne la scelle, le Ciel et la Terre s'en emparent et elle se transforme. L'essence d'un échec de montée prend vie faute d'être reliée à une Fruition : un être contre nature fait d'**essence, de destin et de mauvais présages**, plus fort que tout Manoir Pourpre à son apogée. Seules les forces **pseudo-Noyau d'Or** le soumettent : **Émissaires des Enfers**, **Noyaux Divins**, **Immortels de Jade Tressé**…
  - 📚 **Exemple de *Gongye Shu*** : pendant sa percée, sa capacité de Vie fait apparaître dans les nuages deux **émissaires de la mort** venus **récupérer son essence s'il échoue** ; ils discutent de ses chances (il aurait dû cultiver *Mandat de l'Empereur* plutôt que *Course du Fou vers la Cime*). Il échoue, **dévore sa propre essence** et devient un démon fait de son âme, de son essence et de son destin ; sa chute laisse un vide de pouvoir, ses subordonnés se dispersent et fondent leurs propres territoires.
  - 📚 **Garder une essence est dangereux** : un cultivateur qui en obtint une part lors de la chute d'une Grotte Céleste l'envoya au loin au lieu de la laisser au Monde Souterrain ; elle devint un oiseau de pierre qui dévora un Manoir Pourpre, puis un démon féroce qui en fit tomber un autre dans la Mer du Sud.
  - 📚 **Échec par l'axiome** : un patriarche du Bois Orthodoxe, verrouillé dans sa position orthodoxe par sa capacité de Vie, tenta de s'étendre vers le Bois Rassemblé ; la branche rompit, sa fondation se corrompit en l'essence de la Vertu opposée et il devint un Démon d'Essence Métallique — le cas de référence de « une position orthodoxe ne connaît pas d'Intercalaire ».
- **Voies de la Main Gauche** (✅ L4b : `leftHand` dans `fruitions.json` ; vraie = Grande Perfection de la lignée + voie connue, fait `LeftHand` ; fausse = pouvoir prêté par un détenteur qui a donné sa permission, tribut annuel, chute au Manoir Pourpre si le patron disparaît ou le tribut manque — 🔎 conditions, chances et tribut interprétés) : routes vers une puissance équivalente au Noyau d'Or **sans** position. **Vraies** : autonomes (Immortels au Visage Radieux du Yang Suprême, Immortels de Jade Tressé du Yin Mineur). **Fausses** : dépendantes d'un supérieur (le **Faux Noyau Divin**, dans les Dao Immortel, du Diable et Divin ; les maîtres du Dharma bouddhistes modernes, plafonnés au début du Noyau d'Or, qui empruntent au Qi Brillant, au Yang Lumineux et à la Sentinelle de la Cité les mécanismes laissés par le **Vénérable Kongji**).
...
🎮 ✅ Fait en L4.6 (2026-09-26, demandes de l'utilisateur) :
- **Le savoir est un module** (`World/KnowledgeBase`) : des faits typés (technique, Qi, lignée, capacité, Partenaires Dao, fondation d'un Qi), chacun révélé une fois par une **source** (départ, apprentissage, fondation formée, capacité condensée, étude, miroir, événement, échange, espionnage) et des **implications** branchables (le manuel d'une méthode enseigne son Qi et la fondation qu'il bâtit ; connaître ses Partenaires Dao les révèle). Les règles le consultent : on ne vise qu'une capacité connue, on ne dévore qu'un Partenaire connu. Sources déjà branchées : fondation formée, capacité condensée, déduction, étude (10 % par an pour les Partenaires du savant), faits de départ de `clan.json` ; le miroir, les événements et les échanges s'y brancheront.
- **Ce que le lore ne donne pas se remplace dans les données** : chaque élément porte sa `provenance` (Lore, Wiki, Interpretation) et ses `interpretedFields` ; les valeurs d'interprétation ont quitté le code pour `balance.json` ; `./Scripts/dev.sh gaps` liste tout ce qui reste à confirmer (capacités non révélées, types inconnus, interprétations). Une nouvelle source = une modification de données, sans toucher au code.

🎮 💡 ✅ **Serments du Dao** (L4d, demande de l'utilisateur, 2026-09-26) : deux cultivateurs, quelle que soit leur voie, jurent **sur leur chemin** des clauses (`oaths.json` : ne jamais nuire, garder un secret, ne pas épier, rendre un service avant un terme). Le parjure voit son chemin **interrompu** (progression scellée, parfois une blessure du Dao) ou **ralenti** par un **Démon du Cœur** (cultivation ralentie, stabilité arrachée, qui s'efface avec les années), selon la gravité de la clause (`balance.json`). Rien n'est parfait : **portes dérobées** déclarées dans les données — la lettre plutôt que l'esprit (un tiers agit), le prête-nom, l'échéance, le voile du miroir (au prix de sa puissance), la purification. À venir : la réincarnation ou la conversion comme porte dérobée (L7), les serments des factions (P1), les serments d'autrui appris par l'espionnage ou le miroir. Tout est interprétation (le lore ne cite que les vœux bouddhistes, le pacte ancien de la Renarde des Monts Qingyan et le *Corps du Serment*) : à remplacer dans les données quand une source parle.
...
- **Le Monde Souterrain** (*Tradition de l'Attente de la Clarté*, Douxuan) : fondé par le **Marquis de la Nuit**, « Gouverneur des Âmes » ; il réside dans le **Qi Exilé**, tient les **registres des vivants**, gère la réincarnation et **réclame tous les Démons d'Essence Métallique** et les morts. Lignées : Qi Exilé, Grand Chaman, Qi Véritable, Rite Inférieur, Chouette des Seuils (soupçonnée), Régent Céleste (« en garde »). Il contrôle en sous-main la dynastie mortelle du **Grand Song**, et contrôlait autrefois Linxi. On ignore si son Embryon du Dao existe encore.
  - 📚 **Juridiction sur les Démons d'Essence Métallique** : il revendique une compétence **exclusive** sur ces démons et sur les morts ; ses émissaires assistent aux percées au Noyau d'Or pour **collecter l'essence** en cas d'échec. Selon la coutume de la Profondeur Douxuan, une essence sans maître **doit être laissée au Monde Souterrain** : s'en mêler est vu comme un **abus, voire une provocation**. Organiser une réincarnation ne lui pose aucune difficulté ; ses officiels **peuvent être soudoyés avec de l'essence métallique**. Il détient l'une des essences de l'*Essence du Démon Divin des Cinq Cieux* (Qi Véritable), incarnée dans un héritier mortel qu'il soutient au Grand Song.
